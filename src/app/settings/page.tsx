...
export default function SettingsPage() {
  ...
  return (
    <div className="flex flex-col gap-4 py-4 animate-fade-in">
      ...
      {/* Profile */}
      <div className="rounded-sm bg-bg-secondary p-5 card-depth">
        ...
        <button onClick={logout} className="flex items-center gap-2 rounded-sm bg-danger/10 px-4 py-2.5 text-sm font-bold text-danger hover:bg-danger/20 active:scale-95">
      ...
      <div className="rounded-sm bg-bg-secondary p-5 card-depth">
        ...
        <div className="grid grid-cols-2 gap-2">
          {unitOptions.map((u) => (
            <button key={u} onClick={() => updateSettings({ unit: u })} className={`rounded-sm py-3 text-sm font-bold active:scale-95 ${settings.unit === u ? 'bg-accent text-text-on-accent shadow-[var(--shadow-btn)]' : 'bg-bg-primary text-text-secondary border border-border'}`}>
              {u}
            </button>
          ))}
        </div>
      ...
      <div className="rounded-sm bg-bg-secondary p-5 card-depth">
        ...
        <button onClick={handleExportCSV} className="flex items-center gap-2 rounded-sm border border-border bg-bg-primary px-4 py-2.5 text-sm font-medium text-text-secondary hover:bg-bg-tertiary active:scale-95">
